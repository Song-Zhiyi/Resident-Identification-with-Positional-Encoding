...
def build_preset_from_dat(data: pd.DataFrame, exp_name: str, repeat: int):
    ...
    def _data(data, label, name):
        exps = []
        paths = [
            Path("preprocessed/emb/Exp/node2vec-100-100-256-1"),
            Path("preprocessed/emb/Exp/node2vec-100-100-128-1"),
            Path("preprocessed/emb/Exp/node2vec-200-200-128-1"),
            Path("preprocessed/emb/Exp/node2vec-500-500-128-1"),
        ]
...
def build_preset(exp_name: str, repeat: int = N_REPEATS):
    return (
        build_preset_from_dat(data_prep.dat, exp_name, repeat)
    )
